...
export class Proxy {
  ...
  private onConnect(req: http.IncomingMessage, socket: Duplex, head: Buffer): void {
    ...
    // Границы две, и в отказе важно знать, какая именно сработала: иначе
    // «хост вне разрешённого набора» одинаково выглядит и когда не хватает
    // строки политики узла, и когда хоста нет в наборе токена запуска.
    const inNode = hostKnown(this.nodeAllow(), host);
    const inRun = hostKnown(b.allow, host);
    if (!inNode || !inRun) {
      this.log(b, host, "CONNECT", "-", !inNode && !inRun
        ? "отказ: хост вне политики узла и вне набора запуска"
        : !inNode ? "отказ: хост вне политики узла" : "отказ: хост вне набора запуска");
