...
async function main(): Promise<void> {
  ...
  const nc = await connectAs(`cita-owners@${hostname()}`, SEED, process.env["LPMC_NATS_SERVER"]);
  const state = stopOnSignals();

  await runConsumer(
    nc,
    { stream: "OWNERS", durable: "cita-owners", filterSubject: EVENTS.ownerUpdated.subject },
