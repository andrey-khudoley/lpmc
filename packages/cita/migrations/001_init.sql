-- Схема контрактного исполнителя.
--
-- Она СОВПАДАЕТ по составу со схемой браузерного исполнителя, и это не
-- совпадение, а требование (W1-CITA-01): вокруг действия у обеих модальностей
-- происходит одно и то же — запуск, артефакты, проверка приёмки, маскирование,
-- outbox. Различается только способ действия: браузер против HTTP-клиента.
--
-- Здесь нет ни правил, ни полномочий: исполнитель хранит то, что сделал и что
-- наблюдал, а не то, что ему разрешено. Разрешённое приходит в авторизованной
-- задаче на каждый запуск и не кэшируется — иначе отзыв полномочий не был бы
-- немедленным.
